...
const Products = () => {
    ...
    return (
        <div className="flex flex-col bg-gray-50 min-h-screen pl-6">
            <div className="bg-gray-50 pt-4 pb-4">
                <div className="font-serif font-bold text-3xl mb-6 text-gray-800 border-b-stone-50">
                    <h2 >Inventory Products</h2>
                </div>

                <div className="font-sans flex flex-row mb-6 justify-between">
                    <div className="relative w-full max-w-sm py-1.5">
                        <span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                            <AiOutlineSearch 
                                className={`${isSearching ? 'animate-spin text-green-600' : 'text-gray-400'}`} 
                                size={20} 
                        />
                    </span>
                    <input 
                        ...
                        className="w-65 pl-10 pr-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 bg-white"
                    />
                </div>
                <div >
                    <button 
                        onClick={handleDialogue}
                        className="w-30 h-8 flex items-center justify-center font-bold gap-1 bg-green-600 rounded-lg">
            ...
            <div className="w-full overflow-auto max-h-[60vh] rounded-xl shadow-md bg-white">
                    ...
                    <thead className="bg-gray-100 border-b border-gray-200">
                        <tr>
                            {columns.map((column) => (
                                <th
                                    key={column.id}
                                    className="px-6 py-4 font-bold text-xs uppercase tracking-wider text-gray-600 sticky top-0 bg-gray-100 z-10"
                        ...
                        {rows.length > 0 ? (
                            rows.map((row, index) => (
                                <tr key={row.id || index} className="hover:bg-green-50/50 transition-colors">
                                    {columns.map((column) => (
                                        <td key={column.id} className="px-6 py-2 text-sm text-gray-700 whitespace-nowrap">
                                            {column.id === 'product_image' ? (
                                                ...
                                                <span className="font-medium text-gray-900">{row[column.id]}</span>
                                            ) : (
                                                column.id === 'quantity' && row[column.id] < 10 ? (
                                                    <div className="flex items-center gap-3 py-2 rounded-xl cursor-pointer transition-all duration-300 hover:bg-gray-100 hover:translate-x-1">
                            ...
                            <tr>
                                <td colSpan={columns.length} className="px-6 py-12 text-center text-gray-400 font-sans">
            ...
            <div className="font-[16px] font-semibold mt-4 flex items-center justify-between text-sm text-gray-600 my-2 bg-gray-200 p-2 rounded-lg">
                <p className="bg-white px-4 py-2 rounded-lg font-bold font-italic">Total products: {totalCount}</p>
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="px-3 py-1 rounded border border-none disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                        ...
                        <form className="space-y-4" onSubmit={handleSubmit}>
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <input type="text" 
                                        className="mt-1 w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-green-600" 
                                ...
                                <div className="flex-1">
                                    ...
                                    <input 
                                        type="number" min={1} className="mt-1 w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-green-600" 
                                         onChange={handleChange}
